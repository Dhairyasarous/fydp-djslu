...
//			getPOIAsync(poiTagText.getText().toString());
	...
	public void buildRoadPoints(Road road) {
		// Clear the old route
		route.clear();
	...
	public void checkNextPoint() {
		...
		if (route.size() > 1) {
			...
			if (nodeDistance.checkIfAscendingDistance()) {
				...
				oldNextDistance = distanceToNextNode;
				oldNextNextDistance = distanceToNextNextNode;
			}

		}
		else if (route.size() == 1) {
			double distanceToNextNode = prevLocation.distanceTo(nextNode.mLocation);
			if (distanceToNextNode - oldNextDistance > 2) {
				RoadNode nextNextNode = cloneRoadNode(route.get(0));
				double distanceToNextNextNode = prevLocation.distanceTo(nextNextNode.mLocation);
				Toast.makeText(this, "Route size now empty.", Toast.LENGTH_SHORT).show();
				nextNode = cloneRoadNode(route.get(0));
				oldNextDistance = distanceToNextNextNode;
				route.remove(0);
			}
			...
			if (distanceToNextNode - oldNextDistance > 2) {
				...
				nextNode = null;
				// Send message to bluetooth module
	...
	public RoadNode cloneRoadNode(RoadNode original) {
		...
		clone.mInstructions = original.mInstructions;
		clone.mNextRoadLink = original.mNextRoadLink;
		clone.mLength = original.mLength;
